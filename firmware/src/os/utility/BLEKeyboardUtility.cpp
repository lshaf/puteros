...
void BLEKeyboardUtility::begin()
{
	if (!isInitialized)
	{
		...
		inputMedia = hid->getInputReport(MEDIA_KEYS_ID);
		...
		hid->setReportMap(const_cast<uint8_t*>(_hidReportDescriptor), sizeof(_hidReportDescriptor));
		hid->startServices();
...
void BLEKeyboardUtility::sendReport(MediaKeyReport* keys)
{
	if (this->connected)
	{
		this->printKeyReport(keys);
...
size_t BLEKeyboardUtility::press(const MediaKeyReport k)
{
	const uint16_t k_16 = k[1] | (k[0] << 8);
	uint16_t mediaKeyReport_16 = _mediaKeyReport[1] | (_mediaKeyReport[0] << 8);

	mediaKeyReport_16 |= k_16;
	_mediaKeyReport[0] = static_cast<uint8_t>((mediaKeyReport_16 & 0xFF00) >> 8);
	_mediaKeyReport[1] = static_cast<uint8_t>(mediaKeyReport_16 & 0x00FF);

	sendReport(&_mediaKeyReport);
	return 1;
}

size_t BLEKeyboardUtility::release(uint8_t k)
{
	uint8_t i;
	if (k >= HID_OFFSET) {			// it's a non-printing key (not a modifier)
		k = k - HID_OFFSET;
...
size_t BLEKeyboardUtility::release(const MediaKeyReport k)
{
	const uint16_t k_16 = k[1] | (k[0] << 8);
	uint16_t mediaKeyReport_16 = _mediaKeyReport[1] | (_mediaKeyReport[0] << 8);
	mediaKeyReport_16 &= ~k_16;
...
void BLEKeyboardUtility::releaseAll()
{
	_keyReport.keys[0] = 0;
	_keyReport.keys[1] = 0;
	_keyReport.keys[2] = 0;
...
size_t BLEKeyboardUtility::write(const uint8_t *buffer, size_t size) {
	size_t n = 0;
	while (size--) {
		if (*buffer != '\r') {
			if (write(*buffer)) {
